    ...
    <table class="w-full text-sm rounded-lg table-fixed">
        <thead>
            <tr class="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                ...
                <th class="py-3 px-2 text-left font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap w-40">Borrower Name</th>

                <th class="py-3 px-4 text-left font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap w-34">Dept-Year</th>
                <th class="py-3 px-4 text-left font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap w-24">Status</th>
                <th class="py-3 px-4 text-left font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap w-30">Active Borrows</th>
                <th class="py-3 px-4 text-left font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap w-32 ">Reservations</th>
            ...
            <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-gray-100">
                ...
                <td class="py-2 text-center">
                    ...
                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-accent to-teal-600 flex items-center justify-center text-white font-bold text-xs">
                        {{ strtoupper(substr($borrower->firstname,0,1)) }}{{ strtoupper(substr($borrower->lastname,0,1)) }}
                    </div>
                </td>

                {{-- Borrower Name Column --}}
                <td class="py-3 pl-2 pr-4 whitespace-nowrap ">
                    <div class="font-medium text-gray-700">{{ $borrower->full_name }}
                    </div>
                ...
                <td class="py-3 px-4 text-black whitespace-nowrap text-xs">
                    @if($borrower->role === 'teacher')
                    <div class="font-medium text-gray-700 font-semibold">{{ $borrower->teachers->department->name }}</div>
                    <div class="text-gray-500 text-xs">Faculty</div>
                    @elseif($borrower->role === 'student')
                    <div class="font-medium text-gray-700 font-semibold">{{ $borrower->students->department->name }}</div>
                    @php
                    $yearLevel = $borrower->students->year_level;
                ...
                <td class="py-3 px-4">
                    @if ($borrower->library_status === 'active')
                    ...
                    @elseif ($borrower->library_status === 'suspended')
                    ...
                    @elseif ($borrower->library_status === 'cleared')
                    ...
                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded-xl text-xs font-semibold bg-gray-100 text-black border border-gray-200">
                        ...
                        {{ ucfirst($borrower->library_status) }}
                    </span>
                    @endif
                </td>
                ...
                <td class="py-3 px-4">
                    @if ($borrower->active_borrowings_count > 0)
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full w-full text-xs font-semibold bg-accent/15 text-accent ">
                        ...
                        {{ $borrower->active_borrowings_count }} {{ $borrower->active_borrowings_count === 1 ? 'Book' : 'Books' }}
                    </span>
                    @else
                ...
                <td class="py-3 px-4 align-middle">
                    @php
                    $pendingReservations = $borrower->pendingReservations()->count();
                    $readyReservations = $borrower->readyReservations ? $borrower->readyReservations()->count() : 0;
                    @endphp
...
<div class=" flex flex-col sm:flex-row items-center justify-between mt-6 gap-4">
    <p class="text-sm text-black">
        Showing <span class="font-semibold text-gray-800">{{ $borrowers->firstItem() }}-{{ $borrowers->lastItem() }}</span>
        of <span class="font-semibold text-gray-800">{{ $borrowers->total() }}</span> borrowers
    </p>

    <div class="flex items-center gap-2">
        <!-- Previous -->
        <button class="pagination-btn px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition disabled:opacity-50 disabled:cursor-not-allowed" data-page="{{ $borrowers->currentPage() - 1 }}" {{ $borrowers->onFirstPage() ? 'disabled' : '' }}>
            Previous
        </button>

        <!-- Page Numbers -->
        @foreach ($borrowers->getUrlRange(max(1, $borrowers->currentPage() - 2), min($borrowers->lastPage(), $borrowers->currentPage() + 2)) as $page => $url)
        <button class="pagination-btn px-4 py-2 text-sm font-medium rounded-lg border transition {{ $borrowers->currentPage() == $page ? 'bg-accent text-white border-accent shadow-sm' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}" data-page="{{ $page }}">
            {{ $page }}
        </button>
        @endforeach

        <!-- Next -->
        <button class="pagination-btn px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition disabled:opacity-50 disabled:cursor-not-allowed" data-page="{{ $borrowers->currentPage() + 1 }}" {{ $borrowers->currentPage() == $borrowers->lastPage() ? 'disabled' : '' }}>
            Next
        </button>
    </div>
</div>
